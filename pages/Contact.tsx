...
// ── CHATBOT ───────────────────────────────────────────────────────────────────
type BotStep = 'greeting' | 'name' | 'phone' | 'service' | 'budget' | 'timeline' | 'project' | 'done';
interface BotMsg { from: 'bot' | 'user'; text: string; }
interface BotAnswers { name: string; phone: string; service: string; budget: string; timeline: string; project: string; }

const BOT_FLOW: Record<BotStep, { msg: string; options?: string[]; next?: BotStep }> = {
  greeting: {
    msg: "👋 Hi! I'm the Gamanor assistant. What's your full name?",
    next: 'name',
  },
  name: {
    msg: "Nice to meet you! What's your phone number so we can reach you?",
    next: 'phone',
  },
  phone: {
    msg: "Got it! What service are you interested in?",
...
const ChatBot: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [answers, setAnswers] = useState<BotAnswers>({ name: '', phone: '', service: '', budget: '', timeline: '', project: '' });
  ...
  const reset = () => {
    ...
    setAnswers({ name: '', phone: '', service: '', budget: '', timeline: '', project: '' });
  };
  ...
  return (
    <>
      ...
      `}</style>
      <div
        className="chatbot-box fixed z-50 flex flex-col rounded-2xl border border-[#84C5C9]/30 shadow-[0_0_40px_rgba(132,197,201,0.25)] overflow-hidden bg-[#0B1215]"
        style={{ bottom: '80px', right: '12px', left: '12px', maxHeight: '85vh' }}
      >
        {/* Header */}
        ...
        {/* Messages */}
        <div className="overflow-y-auto p-4 space-y-3 flex-1 bg-[#0B1215]" style={{ minHeight: 0 }}>
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] px-3 py-2 rounded-xl text-sm leading-relaxed ${
                msg.from === 'user'
                  ? 'bg-gradient-to-r from-[#84C5C9]/30 to-[#D4AF37]/20 text-white rounded-br-none'
          ...
          {step !== 'done' && currentFlow.options ? (
            <div className="flex flex-col gap-1.5 max-h-[200px] overflow-y-auto">
              {currentFlow.options.map(opt => (
                <button
                  ...
                  className="flex items-center justify-between w-full px-3 py-2.5 rounded-lg bg-white/[0.04] border border-[#84C5C9]/20 text-slate-300 text-xs font-medium text-left hover:bg-[#84C5C9]/10 hover:border-[#84C5C9]/40 hover:text-white transition-all"
                >
                  <span className="pr-2">{opt}</span>
                  <ChevronRight className="w-3 h-3 shrink-0 text-[#84C5C9] flex-shrink-0" />
                </button>
              ))}
            </div>
          ) : step !== 'done' && !currentFlow.options ? (
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  step === 'greeting' ? "Enter your full name..." :
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg bg-gradient-to-r from-[#84C5C9] to-[#D4AF37] text-black shrink-0"
              >
            ...
            <button
              onClick={reset}
              className="w-full py-2 text-xs font-bold uppercase tracking-widest text-[#84C5C9] border border-[#84C5C9]/30 rounded-lg hover:bg-[#84C5C9]/10 transition-all"
            >
              Start Over
            </button>
          )}
    ...
    <div className="bg-[#0B1215] text-white min-h-screen">
      ...
      <section className="relative flex items-center justify-center px-4 sm:px-6 bg-gradient-to-r from-[#0B1215] to-[#070b0d] overflow-hidden py-16 sm:py-0 sm:min-h-[420px]">
        ...
        <div className="relative z-10 text-center w-full max-w-3xl mx-auto">
          ...
          <p
            className="text-slate-300 text-sm sm:text-base max-w-md mx-auto mb-8 leading-relaxed px-2"
          ...
          <div
            className="flex flex-col items-center sm:flex-row sm:justify-center gap-3 pb-10 sm:pb-0"
            style={{ animation: 'revealUp 1s both 0.4s' }}
          >
            {/* ✅ FIXED: Start a Project now goes to WhatsApp */}
            <a
              href="https://wa.me/919381987790?text=Hi%20Gamanor%2C%20I%20want%20to%20start%20a%20project"
              target="_blank"
              ...
              {submitted ? (
                <div className="text-center py-16">
                  ...
                  <button
                    ...
                    className="mt-8 text-[#84C5C9] font-bold hover:text-[#D4AF37] transition-colors"
                  >
                ...
                <>
                  ...
                  <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      className="w-full bg-gradient-to-r from-[#84C5C9] to-[#D4AF37] text-black font-bold py-3.5 rounded-lg hover:shadow-[0_0_30px_rgba(132,197,201,0.4)] transition-all uppercase tracking-widest text-sm mt-2"
                    >
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-start gap-3">
                    <div className="bg-gradient-to-r from-[#84C5C9]/20 to-[#D4AF37]/20 p-2 rounded-lg shrink-0">
                      {item.icon}
                    </div>
                    <div>
